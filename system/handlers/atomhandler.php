...
class AtomHandler extends ActionHandler
{
	...
	function is_auth( $force = false )
	{
		if ( ( $this->user == null ) || ( $force != false ) ) {
			...
			if ( ( $force != false ) && ( !$this->user->loggedin ) ) {
				...
				header( 'WWW-Authenticate: Basic realm="Filmio"' );
				die();
			}
		}
	...
	public function create_atom_wrapper( $alternate, $self, $id, $updated = null )
	{
		...
		$feed_generator = $xml->addChild( 'generator', 'filmio' );
		$feed_generator->addAttribute( 'uri', 'http://www.Filmioproject.org/' );
		$feed_generator->addAttribute( 'version', Version::get_Filmioversion() );
	...
	public function add_posts( $xml, $posts )
	{
		foreach ( $posts as $post ) {
			...
			$author_uri = $entry_author->addChild( 'uri', Site::get_url( 'filmio' ) );
	...
	public function act_rsd()
	{
		...
		if ( $cache_xml instanceOf SimpleXMLElement ) {
			...
			$service_engine_name = $rsd_service->addChild( 'engineName', 'filmio' );
			$service_engine_link = $rsd_service->addChild( 'engineLink', 'http://www.Filmioproject.org/' );
			$service_home_page_link = $rsd_service->addChild( 'homePageLink', Site::get_url( 'filmio' ) );
