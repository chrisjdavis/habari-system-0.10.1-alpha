...
class UserHandler extends ActionHandler
{
	...
	{
		$name = $form->Filmio_username->value;
		if ( empty($name) ) {
	...
	{
		$name = $form->Filmio_username->value;
		$pass = $form->Filmio_password->value;
	...
	/**
	* function logout
	* terminates a user's session, and deletes the Filmio cookie
	...
	*/
	...
	protected function login_form()
	{
		...
		$form = new FormUI( 'Filmio_login' );
		//$form->on_success( array( $this, 'loginform_success' ) );
		$login_form_title = _t('<h1><a href="%1$s" title="%2$s"><img alt="%2$s" src="%3$s" style="height:1em;margin-right:10px;vertical-align:top;">%4$s</a></h1>', array(Site::get_url('site'), _t('Go to Site'), Site::get_url('filmio', '/system/admin/images/filmio.logo.png'), Options::get('title')));
		...
		$form->append( FormControlLabel::wrap(_t('Name'), FormControlText::create('Filmio_username')->set_property('autofocus', 'autofocus'))->set_template('control.label.outsideleft'));
		$form->append(
