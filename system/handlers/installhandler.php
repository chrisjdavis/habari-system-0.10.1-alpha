...
class InstallHandler extends ActionHandler
{
	...
	public function act_begin_install()
	{
		...
		$this->theme = Themes::create( 'installer', 'RawPHPEngine', FILMIO_PATH . '/system/installer/' );
		 ...
		 * Add the AJAX hooks
		 */
		Plugins::register( Method::create( '\Filmio\InstallHandler', 'ajax_check_mysql_credentials' ), 'ajax_', 'check_mysql_credentials' );
		Plugins::register( Method::create( '\Filmio\InstallHandler', 'ajax_check_pgsql_credentials' ), 'ajax_', 'check_pgsql_credentials' );
		...
		EventLog::log( _t( 'Filmio successfully installed.' ), 'info', 'default', 'filmio' );
		Utils::redirect( Site::get_url( 'site' ) );
	}
	...
	public function form_defaults()
	{
		...
		$formdefaults['db_file'] = 'Filmio.db';
		$formdefaults['db_schema'] = 'filmio';
		$formdefaults['table_prefix'] = isset( Config::get( 'db_connection' )->prefix ) ? Config::get( 'db_connection' )->prefix : 'Filmio__';
		$formdefaults['admin_username'] = 'admin';
		$formdefaults['admin_pass1'] = '';
		$formdefaults['admin_pass2'] = '';
		$formdefaults['blog_title'] = 'My Filmio';
	...
	private function meets_all_requirements()
	{
		...
		if ( extension_loaded( 'pdo' ) ) {
			...
			if ( ! empty( $pdo_drivers ) ) {
				...
				$pdo_schemas = array_map( 'basename', Utils::glob( FILMIO_PATH . '/system/schema/*', GLOB_ONLYDIR ) );
		...
		/**
		 * $local_writable is used in the template, but never set in Filmio
	...
	private function create_default_options()
	{
		...
		EventLog::register_type( 'default', 'filmio' );
		EventLog::register_type( 'user', 'filmio' );
		EventLog::register_type( 'authentication', 'filmio' );
		EventLog::register_type( 'content', 'filmio' );
		EventLog::register_type( 'comment', 'filmio' );
		...
		CronTab::add_daily_cron( 'trim_log', Method::create( '\Filmio\EventLog', 'trim' ), _t( 'Trim the log table' ) );

		// Add the cronjob to check for plugin updates
		CronTab::add_daily_cron( 'update_check', Method::create( '\Filmio\Update', 'cron' ), _t( 'Perform a check for plugin updates.' ) );

		// Add the cronjob to run garbage collection on expired sessions
		CronTab::add_hourly_cron( 'session_gc', Method::create( '\Filmio\Session', 'gc' ), _t( 'Perform session garbage collection.' ) );
	...
	{
		$vocabulary = new Vocabulary( array( 'name' => 'tags', 'description' => 'Filmio\'s tags implementation', 'features' => array( 'multiple', 'free' ) ) );
		$vocabulary->insert();
	...
	private function create_first_post()
	{
		...
		Post::create( array(
			'title' => 'filmio',
			'content' => _t( 'This site is running <a href="http://Filmioproject.org/">Filmio</a>, a state-of-the-art publishing platform!  Filmio is a community-driven project created and supported by people from all over the world.  Please visit <a href="http://Filmioproject.org/">http://Filmioproject.org/</a> to find out more!' ),
			'user_id' => $users[0]->id,
	...
	private function get_create_table_queries( $db_type, $table_prefix, $db_schema )
	{
		...
		$file_path = FILMIO_PATH . "/system/schema/{$db_type}/schema.sql";
	...
	{
		if ( ! ( $file_contents = file_get_contents( FILMIO_PATH . "/system/schema/" . $this->handler_vars['db_type'] . "/config.php" ) ) ) {
			return false;
		}
	...
	/**
	 * returns an array of .htaccess declarations used by Filmio
	 */
	public function htaccess()
	{
		$htaccess = array(
			'open_block' => '### Filmio START',
	...
	public function check_htaccess()
	{
		...
		$result = false;
		if ( file_exists( FILMIO_PATH . '/.htaccess' ) ) {
			$htaccess = file_get_contents( FILMIO_PATH . '/.htaccess' );
			if ( false === strpos( $htaccess, 'filmio' ) ) {
				// the Filmio block does not exist in this file
				// so try to create it
				$result = $this->write_htaccess( true );
			}
			else {
				// the Filmio block exists
				$result = true;
			}
		}
		else {
		...
		if ( $result ) {
			// the Filmio block exists, but we need to make sure
			// it is correct.
	 ...
	 * or to write the Filmio-specific portions to an existing .htaccess
	 * @param bool whether an .htaccess file already exists or not
	 * @param bool whether to remove and re-create any existing Filmio block
	...
	public function write_htaccess( $exists = false, $update = false, $rewritebase = true )
	{
		...
		if ( ! $exists ) {
			if ( ! is_writable( FILMIO_PATH ) ) {
				// we can't create the file
				return false;
			}
		}
		else {
			if ( ! is_writable( FILMIO_PATH . '/.htaccess' ) ) {
		...
		if ( $update ) {
			...
			// care must be take only to remove the Filmio bits
			$htaccess = file_get_contents( FILMIO_PATH . '/.htaccess' );
			$file_contents = preg_replace( '%### Filmio START.*?### Filmio END%ims', $file_contents, $htaccess );
			// Overwrite the existing htaccess with one that includes the modified Filmio rewrite block
			$fmode = 'w';
		}
		else {
			// Append the Filmio rewrite block to the existing file.
			$fmode = 'a';
		}
		// Save the .htaccess
		if ( $fh = fopen( FILMIO_PATH . '/.htaccess', $fmode ) ) {
	...
	/**
	 * returns an array of Files declarations used by Filmio
	 */
	...
	public function secure_sqlite()
	{
		...
		if ( !is_writable( FILMIO_PATH . DIRECTORY_SEPARATOR . '.htaccess' ) ) {
			// we can't update the file
			return false;
		}
		...
		$current_files_contents = file_get_contents( FILMIO_PATH . DIRECTORY_SEPARATOR . '.htaccess' );
		if ( false === strpos( $current_files_contents, $files_contents ) ) {
			// If not, append the files clause to the .htaccess file
			if ( $fh = fopen( FILMIO_PATH . DIRECTORY_SEPARATOR . '.htaccess', 'a' ) ) {
	...
	private function display_currently_upgrading()
	{
		...
		$error_page = sprintf( $error_template,
			...
			_t( "Filmio is currently being upgraded." ), // H1 tag
	...
	private function upgrade_db_post_1310 ()
	{
		...
		if ( ! CronTab::get_cronjob( 'truncate_log' ) ) {
			CronTab::add_daily_cron( 'truncate_log', Method::create( '\Filmio\Utils', 'truncate_log' ), _t( 'Truncate the log table' ) );
		}
	...
	private function upgrade_db_post_1845 ()
	{
		...
		$base_path = array_map( function($s) {return str_replace('\\', '/', $s);}, array( FILMIO_PATH ) );
	...
	private function upgrade_db_post_3749()
	{
		...
		$vocabulary = Vocabulary::create( array( 'name' => 'tags', 'description' => 'Filmio\'s tags implementation', 'features' => array( 'multiple', 'free' ) ) );
		...
		if ( is_array( $valid_plugins ) ) {
			foreach ( $valid_plugins as $filename ) {
				...
				if ( file_exists( $filename ) ) {
					...
					$short_file = substr( $filename, strlen( FILMIO_PATH ) );
					if ( $class ) {
	...
	private function upgrade_db_post_5105 ( ) {
		...
		// generate two new GUIDs -- one that will be used for private (server-side) operations like caching and another that can be used for public (client-side) operations to uniquely identify this Filmio install
	...
	private function upgrade_db_post_5107 ( ) {
		...
		$crons = DB::get_results( 'SELECT * FROM {crontab} WHERE name = ?', array( 'update_check_single' ), '\Filmio\CronJob' );
