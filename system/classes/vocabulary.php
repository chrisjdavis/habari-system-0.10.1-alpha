...
class Vocabulary extends QueryRecord
{
	...
	**/
	...
	public function insert()
	{
		...
		EventLog::log( _t( 'New vocabulary %1$s (%2$s)', array( $this->id, $this->name ) ), 'info', 'content', 'filmio' );
	...
	public function delete()
	{
		...
		EventLog::log( _t( 'Vocabulary %1$s (%2$s) deleted.', array( $this->id, $this->name ) ), 'info', 'content', 'filmio' );
	 ...
	 **/
	public function get_term( $term = null, $term_class = '\Filmio\Term' )
	{
	 ...
	 **/
	public function search_term( $term, $term_class = '\Filmio\Term' )
	{
		$params = array( 'vocab_id' => $this->id );
		$params[ 'term' ] = $term;
	...
	public function merge( $master, $tags, $object_type = 'post' )
	{
		...
		foreach ( $tags as $tag ) {
			...
			if ( $ok_to_delete ) {
				...
				EventLog::log( _t( 'Not all posts tagged "%1$s" could be reassigned to "%2$s". They have been left alone.', array( $tag, $master ) ), 'err', 'vocabulary', 'filmio' );
			}
		...
		EventLog::log( sprintf(
			_n( 'Term %1$s in the %2$s vocabulary has been renamed to %3$s.',
				...
				count( $tags )
			), implode( ', ', $tag_names ), $this->name, $master ), 'info', 'vocabulary', 'filmio'
		);
