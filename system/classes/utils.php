...
class Utils
{
	...
	{
		if ( is_array( $value ) ) {
			$value = array_map( Method::create( '\Filmio\Utils', 'stripslashes' ), $value );
		}
	...
	{
		if ( is_array( $value ) ) {
			$value = array_map( Method::create( '\Filmio\Utils', 'addslashes' ), $value );
		}
	...
	* <code>
	* $mapped_values= array_map(array('\Filmio\Utils', 'map_array'), $values);
	* </code>
	...
	*/
	...
	public static function php_check_file_syntax( $file, &$error = null )
	{
		...
		$replacements = array(
			'#^<\?(php)?\s#A' => '',
			'#\?>$#' => '',
			'#(?<!\w)namespace [^;]+;#' => 'namespace FilmioSandBox\Plugin;'
		);
	...
	{
		return array_reduce( $input, Method::create( '\Filmio\Utils', 'ror' ), 0 );
	}
	...
	*/
	...
	/**
	 * Get a list of the PHP ini settings relevant to Filmio
	 *
