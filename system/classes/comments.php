	...
	public static function moderate_these( $comments, $status = null )
	{
		...
		if ( $comments[0] instanceOf Comment ) {
			...
			foreach ( $comments as $comment ) {
				...
				EventLog::log( _t( 'Comment %1$s moderated from %2$s', array( $comment->id, $comment->post->title ) ), 'info', 'comment', 'filmio' );
			}
		}
		else if ( is_numeric( $comments[0] ) ) {
			$result = true;
			foreach ( $comments as $commentid ) {
				...
				EventLog::log( _t( 'Comment %1$d moderated', array( $commentid ) ), 'info', 'comment', 'filmio' );
			}
		}
		else {
			// We were passed a type we could not understand.
	...
	*/
	...
	public function delete()
	{
		...
		foreach ( $this as $comment ) {
			...
			EventLog::log( _t( 'Comment %1$s deleted from %2$s', array( $comment->id, $comment->post->title ) ), 'info', 'comment', 'filmio' );
		}
		// Clear ourselves.
