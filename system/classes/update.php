 ...
 * Filmio Update class
 *
 * Checks for updates to Filmio and its libraries
 *
 * @access public
 */
class Update extends Singleton
{
	const UPDATE_URL = 'https://beacon.Filmioproject.org/';
	...
	public static function add( $name, $beaconid, $current_version )
	{
		try {
			...
			EventLog::log( _t( 'Beacon updater failed! %s', array($e->getMessage())), 'err', 'update', 'filmio' );
	...
	public static function check()
	{
		...
		try {
			...
			return array_filter( $instance->beacons, Method::create( '\Filmio\Update', 'filter_unchanged' ) );
	...
	public static function cron( $cronjob = null )
	{
		...
		try {
			...
			EventLog::log( _t( 'Updates check CronJob completed successfully.' ), 'info', 'update', 'filmio' );
			...
			EventLog::log( _t( 'Updates check CronJob failed!' ), 'err', 'update', 'filmio', $e->getMessage() );
	 ...
	 * Includes Filmio core, all active plugins, and any pluggable that implements the update_check hook.
	 */
	private static function register_beacons()
	{
		...
		Update::add( 'filmio', '7a0313be-d8e3-11db-8314-0800200c9a66', Version::get_Filmioversion() );
	...
	public static function check_plugins()
	{
		...
		if ( $checked_list != self::instance()->beacons ) {
			...
			CronTab::add_single_cron( 'update_check_single', Method::create( '\Filmio\Update', 'cron' ), DateTime::create()->int, _t( 'Perform a single check for plugin updates, the plugin set has changed.' ) );
	 ...
	 * @return bool false If a single GUID is specified and there are no updates available for it.
	 */
