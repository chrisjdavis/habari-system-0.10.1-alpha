...
class Locale
{
	...
	public static function set( $locale = null )
	{
		...
		self::$uselocale = self::load_domain( 'filmio' );
	}
	...
	private static function load_domain( $domain )
	{
		...
		else if ( file_exists( FILMIO_PATH . '/user/locale/' . $file_end ) ) {
			$file = FILMIO_PATH . '/user/locale/' . $file_end;
		}
		else if ( file_exists( FILMIO_PATH . '/3rdparty/locale/' . $file_end ) ) {
			$file = FILMIO_PATH . '/3rdparty/locale/' . $file_end;
		}
		else {
			$file = FILMIO_PATH . '/system/locale/' . $file_end;
		}
	...
	/**
	 * function list_all
	 * Retrieves an array of the Filmio locales that are installed
	 *
	 * @return array. An array of Filmio locales in the installation
	 */
	public static function list_all()
	{
		$localedirs = array( FILMIO_PATH . '/system/locale/', FILMIO_PATH . '/3rdparty/locale/', FILMIO_PATH . '/user/locale/' );
		...
		$dirs = array_filter( $dirs, function( $a ) {return file_exists($a . "LC_MESSAGES/Filmio.mo"); } );
	...
	{
		$args = func_get_args();
		echo call_user_func_array( Method::create( '\\Filmio\\Locale', '_t' ), $args );
	}
	 ...
	 */
	public static function _t( $text, $args = array(), $domain = 'filmio' )
	{
		if ( is_string( $args ) ) {
			$domain = $args;
		}
	 ...
	 */
	public static function get_messages($domain = 'filmio')
	{
	 ...
	 */
	public static function _u( $text, $domain = 'filmio' )
	{
		$t = $text;
	 ...
	 * @param string $domain (optional) The domain to search for the message
	 */
	public static function _ne( $singular, $plural, $count, $domain = 'filmio' )
	{
	 ...
	 */
	public static function _n( $singular, $plural, $count, $domain = 'filmio' )
	{
