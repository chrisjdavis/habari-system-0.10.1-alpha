 ...
 *
 * The default Filmio ACL class implements groups, and group permissions
...
class ACL
{
	...
	public static function create_token( $name, $description, $group, $crud = false )
	{
		...
		EventLog::log( 'New permission token created: ' . $name, 'info', 'default', 'filmio' );
	...
	public static function destroy_token( $token )
	{
		...
		EventLog::log( _t( 'Permission token deleted: %s', array( $name ) ), 'info', 'default', 'filmio' );
...
SQL;
	...
	public static function grant_group( $group_id, $token_id, $access = 'full' )
	{
		...
		if ( $orig_value != $bitmask->value || ( $orig_value == 0 && !$row_exists && $bitmask->value == 0 ) ) {
			...
			EventLog::log( $msg, 'notice', 'user', 'filmio' );
		}
		else {
			$result = true;
		}
	...
	public static function revoke_group_token( $group_id, $token_id )
	{
		...
		if ( empty( $access ) ) {
			...
			$result = DB::delete( '{group_token_permissions}',
				array( 'group_id' => $group_id, 'token_id' => $token_id ) );
			EventLog::log( _t( 'Group %1$s: Permission to %2$s revoked.', array( $ug->name, ACL::token_name( $token_id ) ) ), 'notice', 'user', 'filmio' );
		}
...
*/
	...
	{
		Plugins::register( array( '\\Filmio\\ACL', '_filter_token_description_display' ), 'filter', 'token_description_display' );
		Plugins::register( array( '\\Filmio\\ACL', '_filter_token_group_display' ), 'filter', 'token_group_display' );
		Plugins::register( array( '\\Filmio\\ACL', '_filter_permission_display' ), 'filter', 'permission_display' );
	}
