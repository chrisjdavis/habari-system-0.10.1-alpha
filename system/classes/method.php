...
class Method
{
	 ...
	 * @return \Filmio\Method The method of the class
	 */
	...
	*/
	...
	{
		// Try the \Filmio namespace if the class doesn't exist and the \Filmio namespace works
		if(!is_object($this->class) && strpos($this->class, '\\') === false && !class_exists($this->class, true)) {
			if(class_exists('\\Filmio\\' . $this->class)) {
				$this->class = '\\Filmio\\' . $this->class;
			}
		}
	 ...
	 * Example:
	 *   $fn = Method::create('Filmio/Utils', 'debug');
	 *   $fn('foo'); // Calls Filmio/Utils::debug('foo');
	 *
