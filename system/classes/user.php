...
class User extends QueryRecord implements FormStorage, IsContent
{
	...
	**/
	...
	public function insert()
	{
		...
		EventLog::log( _t( 'New user created: %s', array( $this->username ) ), 'info', 'default', 'filmio' );
	...
	*/
	public static function authenticate( $who, $pw )
	{
		...
		if ( $user instanceof User ) {
			...
			EventLog::log( _t( 'Successful login for %s', array( $user->username ) ), 'info', 'authentication', 'filmio' );
			// set the cookie
			$user->remember();
		...
		elseif ( !is_object( $user ) ) {
			...
			EventLog::log( _t( 'Login attempt (via authentication plugin) for non-existent user %s', array( $who ) ), 'warning', 'authentication', 'filmio' );
		...
		if ( ! $user ) {
			...
			EventLog::log( _t( 'Login attempt for non-existent user %s', array( $who ) ), 'warning', 'authentication', 'filmio' );
			Session::error( _t( 'Invalid username/password' ) );
			self::$identity = null;
			return false;
		}
		...
		if ( Utils::crypt( $pw, $user->password ) ) {
			...
			EventLog::log( _t( 'Wrong password for user %s', array( $user->username ) ), 'warning', 'authentication', 'filmio' );
			Session::error( _t( 'Invalid username/password' ) );
			self::$identity = null;
			return false;
		}
	...
	public static function commenter()
	{
		...
		if ( self::identify() ) {
			...
			$commenter['url'] = Site::get_url( 'filmio' );
		}
	...
	**/
	public function grant( $tokens, $access = 'full' )
	{
		...
		$tokens = array_map( Method::create( '\Filmio\ACL', 'token_id' ), $tokens );

		foreach ( $tokens as $token ) {
			ACL::grant_user( $this->id, $token, $access );
			EventLog::log( _t( 'User %1$s: Access to %2$s changed to %3$s', array( $this->username, ACL::token_name( $token ), $access ) ), 'notice', 'user', 'filmio' );
		}
	}
	...
	**/
	public function revoke( $tokens )
	{
		...
		foreach ( $tokens as $token ) {
			...
			EventLog::log( _t( 'User %1$s: Permission to %2$s revoked.', array( $this->username, ACL::token_name( $token ) ) ), 'notice', 'user', 'filmio' );
		}
	}
	...
	**/
	public function add_to_group( $group )
	{
		...
		if ( $group instanceOf UserGroup ) {
			$group->add( $this->id );
			EventLog::log( _t( ' User %1$s: Added to %2$s group.', array( $this->username, $group->name ) ), 'notice', 'user', 'filmio' );
		}
	}
	...
	**/
	public function remove_from_group( $group )
	{
		...
		if ( $group instanceOf UserGroup ) {
			...
			EventLog::log( _t( ' User %1$s: Removed from %2$s group.', array( $this->username, $group->name ) ), 'notice', 'user', 'filmio' );
		}
	}
