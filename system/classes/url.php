...
class URL extends Singleton
{
	 ...
	 *
	 * @return \Filmio\RewriteRule matched rule, or null
	 */
	 ...
	 *
	 * @return \Filmio\RewriteRule A rewrite rule that represents a 404 error - no match on the URL requested
	 */
	 ...
	 * @return \Filmio\RewriteRule matched rule, or false
	 */
	...
	public static function get( $rule_names = '', $args = array(), $useall = true, $noamp = false, $prepend_site = true )
	{
		...
		if ( $selectedrule instanceOf RewriteRule ) {
			...
			EventLog::log( _t( 'Could not find a rule matching the following names: %s. File: %s (line %s)', array( implode( ', ', $rule_names ), $error_args['file'], $error_args['line'] ) ), 'notice', 'rewriterules', 'filmio' );
		}
	}
	...
	public static function get_from_filesystem( $path, $trail = false, $preserve_file = false )
	{
		...
		$url = Site::get_url( 'filmio' ) . MultiByte::substr( $path, MultiByte::strlen( FILMIO_PATH ) );
