...
abstract class FormControl
{
	...
	public static function from_args($arglist)
	{
		...
		if(class_exists('\\Filmio\\FormControl' . ucwords($type))) {
			$type = '\\Filmio\\FormControl' . ucwords($type);
		}
...
ERR;
	...
	public function prep_theme(Theme $theme_obj)
	{
		...
		$control_templates_dir = Plugins::filter( 'control_templates_dir', FILMIO_PATH . '/system/controls/templates', $this );
	...
	public function validate()
	{
		...
		foreach ( $this->validators as $validator ) {
			...
			elseif ( FormValidators::have($validator_fn ) ) {
				$validator_fn = Method::create( '\Filmio\FormValidators', $validator_fn );
				$params = array_merge( array( $this->value, $this, $this->get_form() ), $validator );
				$valid = array_merge( $valid, call_user_func_array( $validator_fn, $params ) );
			}
			else {
				$params = array_merge( array( $validator_fn, $valid, $this->value, $this, $this->get_form() ), $validator );
				$valid = array_merge( $valid, call_user_func_array( Method::create( '\\Filmio\\Plugins', 'filter' ), $params ) );
			}
		}
