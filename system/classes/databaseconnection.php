...
/**
 * Filmio DatabaseConnection Class
 *
...
class DatabaseConnection
{
	...
	/**
	 * @var array tables Filmio knows about
	 */
	...
	public static function ConnectionFactory( $connect_string )
	{
		...
		$engines = array(
			'sqlite' => '\Filmio\SQLiteConnection',
			'mysql' => '\Filmio\MySQLConnection',
			'pgsql' => '\Filmio\PGSQLConnection',
		);

		require_once( FILMIO_PATH . "/system/schema/{$engine}/connection.php" );
	...
	/**
	 * Adds a table to the list of tables known to Filmio.  Used
			...
			if ( $this->fetch_mode == \PDO::FETCH_CLASS ) {
				...
				if ( ! class_exists( $this->fetch_class_name, true ) ) {
					...
					if(strpos($class_name, '\\') == false) {
						$class_name = '\\Filmio\\' . $class_name;
																		
						$this->fetch_class_name = $class_name;
						
						if ( ! class_exists( $this->fetch_class_name, true ) ) {
							throw new \Exception('The model class that was specified for data retrieval could not be loaded.');
						}
					}
				}
	 ...
	 */
	public function get_results( $query, $args = array(), $class_name = '\Filmio\QueryRecord' )
	{
	 ...
	 */
	public function get_row( $query, $args = array(), $class_name = '\Filmio\QueryRecord' )
	{
		$this->set_fetch_mode( \PDO::FETCH_CLASS );
		$this->set_fetch_class( $class_name );
