...
class Post extends QueryRecord implements IsContent, FormStorage
{
	...
	public function __construct( $paramarray = array() )
	{
		...
		if ( isset( $this->fields['tags'] ) ) {
			$this->tags_object = Terms::parse( $this->fields['tags'], '\Filmio\Tag', Tags::vocabulary() );
	...
	public function insert()
	{
		...
		EventLog::log( _t( 'New post %1$s (%2$s);  Type: %3$s; Status: %4$s', array( $this->id, $this->slug, Post::type_name( $this->content_type ), $this->statusname ) ), 'info', 'content', 'filmio' );
	...
	public function delete()
	{
		...
		EventLog::log( _t( 'Post %1$s (%2$s) deleted.', array( $this->id, $this->slug ) ), 'info', 'content', 'filmio' );
	...
	public function publish()
	{
		...
		EventLog::log( $msg, 'info', 'content', 'filmio' );
	...
	*/
		...
		switch ( $name ) {
			...
			case 'tags':
				...
				elseif ( is_array( $value ) ) {
					...
					return $this->tags_object = Terms::parse( $value, '\Filmio\Term', Tags::vocabulary() );
				}
			case 'status':
	...
	public function __call( $name, $args )
	{
		...
		return call_user_func_array( Method::create( '\\Filmio\\Plugins', 'filter' ), $args );
	}
	...
	public function comment_form( $context = 'public' )
	{
		...
		if ( $user->loggedin ) {
			...
			$commenter_url = Site::get_url( 'filmio' );
		}
...
// 			$form->class[] = 'comments_require_permission';
	...
	{
		$tokens = Utils::single_array( $tokens );
		$new_tokens = array_map( Method::create( '\Filmio\ACL', 'token_id' ), $tokens );
