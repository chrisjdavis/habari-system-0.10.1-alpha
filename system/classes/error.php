...
namespace Filmio;
/**
 * Contains error-related functions and Filmio's error handler.
 *
 **/
class Error extends \Exception
{
	 ...
	 */
	public function __construct( $message = 'Generic Filmio Error', $code = 0, $is_error = false )
	{
	...
	{
		set_error_handler( Method::create( '\Filmio\Error', 'error_handler' ) );
		set_exception_handler( Method::create( '\Filmio\Error', 'exception_handler' ) );
		register_shutdown_function( Method::create( '\Filmio\Error', 'shutdown_handler' ) );
	}
	...
	public static function error_handler( $errno, $errstr, $errfile, $errline, $errcontext )
	{
		...
		if ( !function_exists( '\Filmio\_t' ) &&  !function_exists( '_t' ) ) {
			function _t( $v )
			{
				return $v;
			}
		...
		$error_names = array(
			...
			// @locale A fatal error generated by Filmio or an addon. Code execution is stopped.
			E_USER_ERROR => _t( 'User Error' ),
			// @locale A non-fatal error generated by Filmio or an addon. Code execution is not stopped.
			E_USER_WARNING => _t( 'User Warning' ),
			// @locale PHP encountered something generated by Filmio or an addon at runtime that could be an error or intended
			...
			// @locale A notification that the code is outdated and Filmio may not work with it in the future
			E_USER_DEPRECATED => _t( 'User deprecated violation' ),
		);
		
		if ( strpos( $errfile, FILMIO_PATH ) === 0 ) {
			$errfile = substr( $errfile, strlen( FILMIO_PATH ) + 1 );
		}
