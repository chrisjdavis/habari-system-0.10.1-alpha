		...
		if ( !isset( $themes[$theme_dir] ) ) {
			...
			foreach ( $themes as $themedir ) {
				if ( file_exists( Utils::end_in_slash( $themedir ) . 'theme.xml' ) ) {
					...
					EventLog::log( _t( "Active theme directory no longer available.  Falling back to '{$theme_dir}'" ), 'err', 'theme', 'filmio' );
	...
	**/
	public static function activate_theme( $theme_name, $theme_dir )
	{
		...
		if($ok) {
			...
			EventLog::log( _t( 'Activated Theme: %s', array( $theme_name ) ), 'notice', 'theme', 'filmio' );
		}
		return $ok;
	}
	...
	public static function preview_theme( $theme_name, $theme_dir )
	{
		...
		if($ok) {
			...
			EventLog::log( _t( 'Previewed Theme: %s', array( $theme_name ) ), 'notice', 'theme', 'filmio' );
		}
	...
	public static function cancel_preview()
	{
		if ( isset( $_SESSION['user_theme_name'] ) ) {
			...
			EventLog::log( _t( 'Canceled Theme Preview: %s', array( $_SESSION['user_theme_name'] ) ), 'notice', 'theme', 'filmio' );
	...
	public static function create( $name = null, $template_engine = null, $theme_dir = null )
	{
		...
		$fallback_classname = '\\Filmio\\Theme';
	...
	public static function get_theme_classes()
	{
		...
		do {
			...
			foreach($class_parents as $class => $parents) {
				if(count(array_intersect($theme_classes + array('Filmio\\Theme'), $parents))>0) {
