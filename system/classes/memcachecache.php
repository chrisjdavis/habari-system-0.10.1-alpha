...
class MemcacheCache extends Cache
{
	...
	public function __construct()
	{
		...
		if ( $this->enabled ) {
			...
			$this->cache_index = $this->memcache->get('Filmio:cache:index');
		}
		else {
	...
	protected function _set( $name, $value, $expiry, $group, $keep )
	{
		...
		$this->clear_expired();
		$this->memcache->set('Filmio:cache:index', $this->cache_index, null, 0);
	...
	protected function _extend( $name, $expiry, $group )
	{
		...
		if ( isset( $this->cache_index[$group][$name] ) ) {
			...
			$this->clear_expired();
			$this->memcache->set('Filmio:cache:index', $this->cache_index);
		}
