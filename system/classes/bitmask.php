...
class Bitmask
{
	...
	{
		if ( ! is_array( $flags ) ) {
			// @locale Filmio tried to create a Bitmask with the wrong arguments
		...
		if ( count( $flags ) > ( PHP_INT_MAX >> 1 ) ) {
			// @locale Filmio tried to create a new Bitmask with too many flags
		...
		if ( ! is_null( $value ) ) {
			if( !is_string( $value ) && !is_int( $value ) ) {
				// @locale Filmio tried to create a new Bitmask object with invalid values in the second argument
	...
	public function __set( $bit, $on )
	{
		switch ( $bit ) {
			case 'full':
				if ( ! is_bool( $on ) ) {
					// @locale Filmio tried to set the wrong type of value
			...
			case 'value':
				if ( is_array( $on ) ) {
					if ( count( $on ) !== count( $this->flags ) ) {
						// @locale Filmio tried to set the wrong kind of value
			...
			default:
				...
				if ( $bit === false ) {
					// @locale Filmio tried to set a value that doesn't exist
	...
	public function __get( $bit )
	{
		...
		elseif ( ( $bit = array_search( $bit, $this->flags ) ) !== false ) {
			...
			// @locale Filmio tried to get a flag that doesn't exist
