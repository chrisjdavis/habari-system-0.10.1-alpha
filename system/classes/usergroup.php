...
class UserGroup extends QueryRecord
{
	...
	public function insert()
	{
		...
		EventLog::log( _t( 'New group created: %s', array( $this->name ) ), 'info', 'default', 'filmio' );
	...
	public function update()
	{
		...
		EventLog::log( _t( 'User Group updated: %s', array( $this->name ) ), 'info', 'default', 'filmio' );
	...
	protected function set_member_list()
	{
		...
		EventLog::log( _t( 'User Group %s: Member list reset', array( $this->name ) ), 'notice', 'user', 'filmio' );
	}
	...
	public function delete()
	{
		...
		EventLog::log( _t( 'User Group %s: Group deleted.', array( $name ) ), 'notice', 'user', 'filmio' );
		return $result;
	}
	...
	public function add( $users )
	{
		...
		$user_ids = array_map( Method::create( '\Filmio\User', 'get_id' ), $users );
		...
		EventLog::log( _t( 'User Group %1$s: Users were added to the group.', array( $this->name ) ), 'notice', 'user', 'filmio' );
	}
	...
	public function remove( $users )
	{
		...
		$users = Utils::single_array( $users );
		// Use ids internally for all users
		$users = array_map( Method::create( '\Filmio\User', 'get_id' ), $users );
		// Remove users from group membership
		$this->member_ids = array_diff( $this->member_ids, $users );
		$this->update();

		EventLog::log( _t( 'User Group %1$s: Users were removed from the group.', array( $this->name ) ), 'notice', 'user', 'filmio' );
	}
