...
');
	...
	{
		// set the locale and character set that Filmio is configured to use presently
	...
	public function theme_header( $theme )
	{
		...
		$styles = Stack::get( 'template_stylesheet', Method::create( '\\Filmio\\Stack', 'styles' ) );
		$scripts = Stack::get( 'template_header_javascript', Method::create( '\\Filmio\\Stack', 'scripts' ) );
	...
	public function theme_footer( $theme )
	{
		...
		$output = Stack::get( 'template_footer_stylesheet', Method::create( '\\Filmio\\Stack', 'styles' ) );
		$output .= Stack::get( 'template_footer_javascript', Method::create( '\\Filmio\\Stack', 'scripts' ) );
		return $output;
	}
	...
	public function theme_next_page_link( $theme, $text = null, $classes = array( 'next-page' ) )
	{
		...
		var_dump($theme->posts->count_all()); exit();
	...
	public function __call( $function, $params )
	{
		if ( strpos( $function, 'act_' ) === 0 ) {
			...
			$result = call_user_func_array( Method::create( '\\Filmio\\Plugins', 'theme' ), $params );
			switch ( $purposed ) {
				case 'return':
					return $result;
				case 'end':
	...
	**/
	...
	public function dir_to_url($dir)
	{
		...
		if(!$tomatch) {
			$tomatch = array(
				...
				FILMIO_PATH . '/user/themes/' => Site::get_url( 'filmio' ) . '/user/themes/',
				FILMIO_PATH . '/3rdparty/themes/' => Site::get_url( 'filmio' ) . '/3rdparty/themes/',
				FILMIO_PATH . '/system/themes/' => Site::get_url( 'filmio' ) . '/system/themes/',
				FILMIO_PATH . '/system/admin/' => Site::get_url( 'filmio' ) . '/system/admin/',
			);
		}
	...
	/**
	 * Provide default Filmio features for curious plugins
	 * @param array $provided Features already collected from interrogated plugins
	 * @return array Plugin Features plus Filmio Features
	 */
	public static function filter_provided( $provided = array() ) {
		foreach( array( Version::Filmio_MAJOR_MINOR, DB::get_driver_name(),
			) as $feature ) {
			$provided[ $feature ] = array( "Filmio" );
		}
		return $provided;
	}
