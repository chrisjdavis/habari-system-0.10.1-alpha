...
class Plugins
{
	...
	public static function theme($hookname, $param)
	{
		...
		if ( !isset( self::$hooks['theme'][$hookname] ) ) {
			if ( substr( $hookname, -6 ) != '_empty' ) {
				...
				return call_user_func_array( Method::create( '\\Filmio\\Plugins', 'theme' ), $filter_args );
			}
			return array();
		}
		...
		if ( count( $return ) == 0 && substr( $hookname, -6 ) != '_empty' ) {
			array_unshift( $filter_args, $hookname . '_empty' );
			$result = call_user_func_array( Method::create( '\\Filmio\\Plugins', 'theme' ), $filter_args );
		}
		array_unshift( $filter_args, 'theme_call_' . $hookname, $return );
		$result = call_user_func_array( Method::create( '\\Filmio\\Plugins', 'filter' ), $filter_args );
		return $result;
	}
			...
			if ( is_array( $plugins ) ) {
				foreach ( $plugins as $class => $filename ) {
					...
					if ( $class == '' ) {
						...
						EventLog::log( _t( 'An empty plugin definition pointing to file "%1$s" was removed.', array( $filename ) ), 'err', 'plugin', 'filmio' );
					...
					if ( file_exists( $filename ) ) {
						...
						self::deactivate_plugin( $filename, true );
						EventLog::log( _t( 'Plugin "%1$s" deactivated because it could no longer be found.', array( $class ) ), 'err', 'plugin', 'filmio', $filename );
					}
				}
			}
	...
	*/
	...
	{
		$plugins = array();
		$plugindirs = array( FILMIO_PATH . '/system/plugins/', FILMIO_PATH . '/3rdparty/plugins/', FILMIO_PATH . '/user/plugins/' );
	...
	public static function extends_plugin( $class )
	{
		...
		$matches = array_intersect(array('Plugin', 'Filmio\Plugin'), $parents);
	...
	public static function get_plugin_classes()
	{
		...
		return array_filter( $classes, Method::create( '\Filmio\Plugins', 'extends_plugin' ) );
	}
	 ...
	 * @return \Filmio\Plugin The instantiated plugin class
	 */
	...
	public static function activate_plugin( $file )
	{
		...
		if(preg_match('#^([^:]+://)#i', $file, $matches)) {
			...
			// trim off the leading filmio path from the filename
			$short_file = MultiByte::substr( $file, strlen( FILMIO_PATH ) );
		}
		...
		if ( $ok ) {
			...
			EventLog::log( _t( 'Activated Plugin: %s', array( $plugin->info->name ) ), 'notice', 'plugin', 'filmio' );
		}
		return $ok;
	}
	...
	public static function deactivate_plugin( $file, $force = false )
	{
		...
		if ( $ok || $force == true ) {
			...
			$short_file = MultiByte::substr( $file, MultiByte::strlen( FILMIO_PATH ) );
			...
			if ( is_array( $activated ) && ( false !== $index ) ) {
				...
				if ( $force != true ) {
					...
					EventLog::log( _t( 'Deactivated Plugin: %s', array( $name ) ), 'notice', 'plugin', 'filmio' );
				}
			}
		}
	...
	public static function changed_since_last_activation()
	{
		...
		foreach ( $old_plugins as $old_plugin ) {
			$old_plugin = FILMIO_PATH . $old_plugin;
		}
	...
	public static function set_present()
	{
		...
		foreach ( $plugin_files as $plugin_file ) {
			$plugin_file = MultiByte::substr( $plugin_file, MultiByte::strlen( FILMIO_PATH ) );
		}
