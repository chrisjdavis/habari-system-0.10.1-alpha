		...
		if(!$this->get_setting('is_dupe', false)) {
			...
			if( isset( $_POST['_form_id'] ) && $_POST['_form_id'] == $this->control_id()) {
				...
				if ( !$this->has_session_data() || !isset( $_SESSION['forms'][$this->control_id()]['url'] ) ) {
					$_SESSION['forms'][$this->control_id()]['url'] = Site::get_url( 'filmio', true ) . Controller::get_stub() . '#' . $this->get_id(false);
				}
			}
...
//		$this->options['ajax'] = true;
		...
		$this->properties['onsubmit'] = "Filmio.media.submitPanel('$path', '$panel', this, '{$callback}');return false;";
	}
