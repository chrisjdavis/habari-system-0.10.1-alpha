...
class Autoload
{
	 ...
	 */
	public static function Filmio_autoload( $class_name )
	{
		$success = false;
		$full_class_name = $class_name;
			
		if(!preg_match('#^\\\\?Filmio\\\\#', $class_name)) {
			return false;
		}

		$class_name = preg_replace('#^\\\\?Filmio\\\\#', '', $class_name);
				
		$class_file = strtolower( $class_name ) . '.php';

		if ( empty( self::$files ) ) {
			$dirs = array(
				FILMIO_PATH . '/system/classes',
				FILMIO_PATH . '/system/controls',
				FILMIO_PATH . '/system/handlers',
				FILMIO_PATH . '/user/classes',
				FILMIO_PATH . '/user/controls',
				FILMIO_PATH . '/user/handlers',
			);
			...
			// Verify if this Filmio instance is a multisite.
			if ( ( $site_user_dir = Site::get_dir( 'user' ) ) != FILMIO_PATH . '/user' ) {
	...
	/**
	 * SPL Nested Autoload function, includes a file to meet requirement of loading a class by name
	 * @param string $class_name The name of a class, including mutli-level namespace
	 * @return bool True if this function successfully autoloads the class in question
	 */
	public static function Filmio_nested_autoload( $class_name )
	{
		$success = false;
		$full_class_name = $class_name;
