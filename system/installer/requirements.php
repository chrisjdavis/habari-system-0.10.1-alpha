...
<div id="header">
	<h1><?php _e('Before you install <em>Filmio</em>&hellip;'); ?></h1>
</div>
<?php if (! $local_writable) {?>
	<h2><?php _e('Writable directory needed'); ?>&hellip;</h2>
	<?php if ($PHP_OS != 'WIN') {?>
		<p class="instructions">
			<?php _e('Before you can install Filmio, you first need to make the install directory writable by php, so that the installation script can write your configuration information properly. The exact process depends on the web server and the ownership of the directory.'); ?>
		</p>
		<p>
		...
		<ol>
			<li>
				...
				<pre><strong>$&gt;</strong> cd <?php echo $FILMIO_PATH;?></pre>
			</li>
			<li>
...
<?php if (! $php_version_ok) {?>
	...
	<p class="instructions">
		<em>Filmio</em> <?php _e('requires PHP %s or newer. Your current PHP version is %s.', array(MIN_PHP_VERSION, PHP_VERSION)); ?>
	</p>
...
?>
	...
	<p class="instructions">
		<em>Filmio</em> <?php _e('requires the following PHP extensions to be installed:'); ?> <?php echo $missing_ext_html; ?>. <?php _e('Please contact your web hosting provider if you do not have access to your server.'); ?>
	</p>
<?php }?>
...
<?php if ( extension_loaded( 'pcre' ) && ! @preg_match( '/\p{L}/u', 'a' ) ) { ?>
	...
	<p class="instructions">
		<em>Filmio</em> <?php _e('requires PHP\'s PCRE extension to have Unicode support enabled.'); ?> <?php _e('Please contact your web hosting provider if you do not have access to your server.'); ?>
	</p>
...
<?php if ( ! $pdo_drivers_ok && ! array_key_exists( 'pdo', $missing_extensions )  ) { ?>
	...
	<p class="instructions"><em>Filmio</em> <?php _e('requires that at least one <a href="http://www.php.net/pdo">PDO driver</a> be installed. Please ask your hosting provider to enable one of the PDO drivers supported by Filmio.'); ?></p>
<?php } ?>

<?php if ( ! $mod_rewrite ) { ?>
	<h2><?php _e('mod_rewrite not enabled'); ?></h2>
	<p class="instructions"> <?php _e('You have Apache, but mod_rewrite is not enabled. To use Filmio, you must enable mod_rewrite in your server configuration. <strong>mod_rewrite</strong> is an Apache module that allows Filmio (and other software) to handle URLs dynamically. You need a <code>LoadModule rewrite_module</code> line in your <code>httpd.conf</code>. Please ask your systems administrator or hosting provider to enable mod_rewrite.'); ?></p>
<?php } ?>
