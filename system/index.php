...
if ( !defined( 'FILMIO_PATH' ) ) {
	header( 'HTTP/1.1 403 Forbidden', true, 403 );
	die('If you are seeing this message, and you were expecting to see your Filmio install, you probably cloned the wrong repo.');
}

// Compares PHP version against our requirement.
if ( !version_compare( PHP_VERSION, '5.4', '>=' ) ) {
	die( 'Filmio needs PHP 5.4 or higher to run. You are currently running PHP ' . PHP_VERSION . '.' );
}

// Increase the error reporting level: E_ALL, E_NOTICE, and E_STRICT
error_reporting( E_ALL | E_NOTICE | E_STRICT );

// set a default timezone for PHP. Filmio will base off of this later on
...
spl_autoload_register( array('\Filmio\Autoload', 'filmio_autoload') );
spl_autoload_register( array('\Filmio\Autoload', 'Filmio_nested_autoload') );
...
if ( Config::exists('db_connection') ) {
	...
	try {
		DB::connect();
		// Make sure Filmio is installed properly.
		...
		$error_page = sprintf( $error_template,
			_t( "Filmio General Error" ), // page title
...
/* Filmio is installed and we established a connection with the database */
...
spl_autoload_register( Method::create( '\Filmio\Plugins' , '_autoload' ) );
Plugins::load_active();
