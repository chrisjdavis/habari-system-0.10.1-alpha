// Filmio ajax. All Filmio Ajax calls should go through here. It allows us to use uniform humanmsg stuff,
// as well as uniform error handling
var Filmio_ajax = {
	post: function(post_url, post_data, ahah_target, local_cb) {
		Filmio_ajax.ajax('POST', post_url, post_data, ahah_target, local_cb);
	},
	
	get: function(get_url, get_data, ahah_target, local_cb) {
		Filmio_ajax.ajax('GET', get_url, get_data, ahah_target, local_cb);
	},
	...
	ajax: function(type, url, data, ahah_target, local_cb) {
		$.ajax({
			...
			success: function(json_data) {
				...
				Filmio_ajax.success(json_data, cb);
			},
		error: Filmio_ajax.error,
		  dataType: 'json',
			type: type
		});
	},
	...
	success: function(json_data, local_cb)
	{
		...
		if ( json_data.response_code >= 400 ) {
			Filmio_ajax.error(null, json_data.message, json_data.response_code)
		}
		if(json_data.Filmio_callback != null && json_data.Filmio_callback != '') {
			json_data.Filmio_callback = eval(json_data.Filmio_callback);
			if($.isFunction(json_data.Filmio_callback)) {
				json_data.Filmio_callback(json_data);
			}
		}
...
var spinner = {
	start: function() {
		$('#spinner')
			...
			.css('background-image', 'url(' + Filmio.url.Filmio + '/system/admin/images/spin.gif)')
			.show();
	},
	stop: function () {
		$('#spinner').hide();
...
$(document).ready(function(){
	...
	$(window).load(function(){
		window.setTimeout(function(){
			labeler.check($('label[for=Filmio_password]'));
		}, 200);
	});
